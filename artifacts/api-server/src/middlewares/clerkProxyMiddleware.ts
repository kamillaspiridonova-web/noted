  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        // Resolve the real client IP so Clerk applies per-user rate limits
        // rather than rate-limiting all users against the shared server IP.
        // Check headers in priority order: X-Forwarded-For (set by Replit's
        // CDN/edge), then X-Real-IP (set by nginx-based proxies), then fall
        // back to the socket address as a last resort.
        const xff = req.headers["x-forwarded-for"];
        const xffIp = (Array.isArray(xff) ? xff[0] : xff)
          ?.split(",")[0]
          ?.trim();
